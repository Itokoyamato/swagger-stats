...
swsElasticEmitter.prototype.flush = function(){
    ...
    var options = {
        ...
        host: `${process.env.AWS_ES_CLUSTER_ID}.${process.env.AWS_ES_REGION}.es.amazonaws.com`,
        service: 'es',
        path: '/_bulk'
    };
    let signed_request = aws4.sign(options, {accessKeyId: process.env.AWS_ES_ACCESS_KEY, secretAccessKey: process.env.AWS_ES_SECRET_KEY});

    request.post((process.env.AWS_ES_URL) ? signed_request : options, function (error, response, body) {
        if (error) {
